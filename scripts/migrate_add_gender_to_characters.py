"""
MongoDB characters 컬렉션에 gender 필드 추가 마이그레이션
...
Usage:
  ...
  # 실행
  python scripts/migrate_add_gender_to_characters.py
"""
...
def main():
    ...
    client = MongoClient(
        uri,
        appname="arcanaverse-gender-migration",
        tls=True,
    ...
    # gender 필드가 없는 문서들에 gender: "none" 추가
    ...
    # 검증: gender 필드가 없는 문서가 남아있는지 확인
    remaining = collection.count_documents({"gender": {"$exists": False}})
    if remaining > 0:
        print(f"  WARNING: {remaining} documents still missing gender field")
    else:
        print(f"  ✓ All documents now have gender field")
