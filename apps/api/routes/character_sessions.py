# ========================================
# apps/api/routes/character_sessions.py — 캐릭터 세션 API
# - POST /v1/character-sessions/{session_id}/persona : 세션에 페르조나 적용
# ========================================
...
class CharacterSessionPersonaApplyRequest(BaseModel):
    """캐릭터 세션에 페르조나 적용 요청"""
    persona_id: str


class CharacterSessionPersonaApplyResponse(BaseModel):
    """캐릭터 세션에 페르조나 적용 응답"""
    ok: bool
    session_id: str
...
@router.post("/{session_id}/persona", summary="캐릭터 세션에 페르조나 적용", response_model=CharacterSessionPersonaApplyResponse)
async def apply_persona_to_character_session(
    session_id: str,
    payload: CharacterSessionPersonaApplyRequest,
    ...
    - characters_session 문서에 persona 스냅샷 저장
    """
    try:
        ...
        logger.info(
            "[PERSONA][APPLY] session_id=%s user_id=%s persona_id=%s",
            session_id,
            user_id,
            payload.persona_id
        )
        
        return CharacterSessionPersonaApplyResponse(
            ok=True,
        ...
        logger.exception("[PERSONA][APPLY][ERROR] session_id=%s error=%s", session_id, str(e))
