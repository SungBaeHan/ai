...
def persist_game_chat(
    ...
    try:
        ...
        # 1) games_session 컬렉션에 세션 저장/업데이트 (upsert)
        ...
        if world_id:
            session_update["$set"]["world_id"] = world_id
        session_result = session_col.update_one(session_filter, session_update, upsert=True)
        writes["session"] = {
            "matched": session_result.matched_count,
            "modified": session_result.modified_count,
        ...
        logger.info(
            "[CHAT][DB] trace=%s col=games_session op=update_one matched=%d modified=%d upserted_id=%s",
            trace_id,
            session_result.matched_count,
            session_result.modified_count,
            str(session_result.upserted_id) if session_result.upserted_id else None,
        ...
        # 2) games_message 컬렉션에 사용자 메시지 저장
        ...
        logger.info(
            "[CHAT][DB] trace=%s col=games_message op=insert_one role=user inserted_id=%s",
            trace_id,
            str(user_msg_result.inserted_id),
        )
        
        # 3) games_message 컬렉션에 어시스턴트 메시지 저장
        assistant_message_doc = {
            "session_id": session_id,
            "user_id": user_id,
            "role": "assistant",
        ...
        logger.info(
            "[CHAT][DB] trace=%s col=games_message op=insert_one role=assistant inserted_id=%s",
            trace_id,
            str(assistant_msg_result.inserted_id),
        )
        
        # 4) games_event 컬렉션에 이벤트 저장 (선택사항)
        ...
        logger.info(
            "[CHAT][DB] trace=%s col=games_event op=insert_one inserted_id=%s",
            trace_id,
            str(event_result.inserted_id),
        )
